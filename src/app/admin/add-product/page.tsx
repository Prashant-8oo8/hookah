...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setName(""); setDescription(""); setPrice(""); setCategory(""); setStock(""); setColor(""); setFile(null);
    } catch (error: any) {
      ...
      <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl bg-background p-6 rounded-lg border border-border/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-3">
            ...
            <Input id="price" type="number" required min="0" value={price} onChange={(e) => setPrice(e.target.value)} className="bg-muted/50 h-12" />
          </div>
          <div className="space-y-3">
            <Label htmlFor="stock" className="text-base">Stock Quantity</Label>
            <Input id="stock" type="number" required min="0" value={stock} onChange={(e) => setStock(e.target.value)} className="bg-muted/50 h-12" />
          </div>
        </div>
